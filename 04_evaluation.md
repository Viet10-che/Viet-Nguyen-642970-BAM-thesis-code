...
Tested using mean absolute SHAP values from RF Block 3 and XGB Block 3. A higher mean
|SHAP| for `f_sell_5d` relative to `f_buy_5d` supports H3a. Global importance and signed
SHAP (mean SHAP without taking absolute value) are reported for all nine Block 3 features.
Unsigned importance identifies which features matter most. Meanwhile signed importance identifies
the average direction of each feature's contribution.

Note: FE coefficient comparison is omitted as saving 514 × 210 coefficient vectors adds
complexity for minimal insight.
...
## H3a Supplement: Signed SHAP Average Contribution

**H3a:** The directional contribution of each Block 3 feature is examined using mean
signed SHAP values (without taking absolute value).

The signed average shows whether each feature systematically pushes predictions upward
or downward on average across all OOS observations. A feature with large unsigned
importance but near-zero signed mean operates bidirectionally depending on context.
Results are reported for RF Block 3 and XGB Block 3.


```python
print("H3a: signed SHAP average contribution — RF and XGB Block 3")

signed_results = {}

for model_name, shap_data in [("RF", shap_rf), ("XGB", shap_xgb)]:
    signed_imp = shap_data[FEATURE_COLS].mean().reset_index()
    signed_imp.columns = ["feature", "mean_signed_shap"]
    signed_imp["mean_abs_shap"] = shap_data[FEATURE_COLS].abs().mean().values
    signed_imp = signed_imp.sort_values("mean_abs_shap", ascending=False).reset_index(drop=True)
    signed_results[model_name] = signed_imp

    print(f"\n{model_name} Block 3")
    print(f"{'Feature':<22} {'Mean Signed SHAP':>18} {'Mean Abs SHAP':>14} {'Avg sign':>10}")

    for _, row in signed_imp.iterrows():
        direction = "positive" if row["mean_signed_shap"] > 0 else ("negative" if row["mean_signed_shap"] < 0 else "zero")
        print(
            f"  {row['feature']:<20} "
            f"{row['mean_signed_shap']:>+18.6f} "
            f"{row['mean_abs_shap']:>14.6f} "
            f"{direction:>10}"
        )

for model_name, df_signed in signed_results.items():
    out_path = f"output/results/h3a_shap_signed_{model_name.lower()}.csv"
    df_signed.to_csv(out_path, index=False)
    print(f"Saved: {out_path}")
```

    H3a: signed SHAP average contribution — RF and XGB Block 3
    
    RF Block 3
    Feature                  Mean Signed SHAP  Mean Abs SHAP   Avg sign
      ret_1d_lag                    -0.000007       0.000531   negative
      momentum_1m                   +0.000007       0.000494   positive
      f_sell_5d                     +0.000036       0.000348   positive
      ret_5d_lag                    -0.000010       0.000166   negative
      log_size                      -0.000001       0.000140   negative
      amihud_5d                     +0.000001       0.000126   positive
      volatility_1m                 -0.000002       0.000079   negative
      f_buy_5d                      -0.000003       0.000063   negative
      turnover_5d                   -0.000000       0.000054   negative
    
    XGB Block 3
    Feature                  Mean Signed SHAP  Mean Abs SHAP   Avg sign
      ret_1d_lag                    +0.000001       0.000915   positive
      momentum_1m                   +0.000008       0.000897   positive
      f_sell_5d                     +0.000014       0.000550   positive
      ret_5d_lag                    -0.000023       0.000466   negative
      amihud_5d                     +0.000002       0.000344   positive
      log_size                      -0.000006       0.000293   negative
      f_buy_5d                      -0.000032       0.000213   negative
      turnover_5d                   +0.000003       0.000156   positive
      volatility_1m                 -0.000002       0.000142   negative
    Saved: output/results/h3a_shap_signed_rf.csv
    Saved: output/results/h3a_shap_signed_xgb.csv


## Signed SHAP Quintile Diagnostic for Foreign Flow Variables

**H3a:** The directional channel of foreign flow features is examined by sorting
observations into quintiles of actual feature value and computing mean signed SHAP
within each quintile.

The directional test is conducted only for foreign flow variables (`f_buy_5d`,
`f_sell_5d`) because these are the only Block 3 variables with genuine directional
ambiguity between two competing theoretical channels: the information channel
predicts that high foreign buying precedes positive returns, while the price pressure
reversal channel predicts the opposite. Liquidity variables (`turnover_5d`,
`amihud_5d`) are excluded because their directional relationship with short-horizon
returns is established in prior literature and not theoretically contested here.

Note: Feature values are read from `model_ready.csv`, which contains
post-winsorization, pre-rank-normalization values. This ensures quintiles reflect
the pre-transformation distribution seen by the model.


```python
print("H3a: signed SHAP quintile diagnostic — foreign flow variables")

flow_features = ["f_sell_5d", "f_buy_5d"]

model_ready = pd.read_csv("output/intermediate/model_ready.csv", parse_dates=["date"])
feature_values = model_ready[["ticker", "date"] + flow_features].copy()
...
for model_name, shap_data in [("RF", shap_rf), ("XGB", shap_xgb)]:
    ...
    for feature in flow_features:
        tmp = shap_data[["ticker", "date", feature]].copy()
        tmp = tmp.rename(columns={feature: "signed_shap"})
        tmp = tmp.merge(feature_values[["ticker", "date", feature]], on=["ticker", "date"], how="left")
        ...
        summary = (
            tmp.groupby("quintile", observed=False)
            .agg(
                mean_feature_value=(feature, "mean"),
                mean_signed_shap=("signed_shap", "mean"),
                mean_abs_shap=("signed_shap", lambda x: x.abs().mean()),
                n_obs=("signed_shap", "size")
            )
            .reset_index()
        )

        q1_signed = summary.loc[summary["quintile"] == "Q1 low",  "mean_signed_shap"].iloc[0]
        q5_signed = summary.loc[summary["quintile"] == "Q5 high", "mean_signed_shap"].iloc[0]

        if q5_signed > 0:
            high_value_direction = "high values push predictions upward"
        elif q5_signed < 0:
            high_value_direction = "high values push predictions downward"
        else:
            high_value_direction = "high values have no average directional effect"

        print(f"\n  Feature: {feature}  |  high-value direction: {high_value_direction}")
        print(summary.round(6).to_string(index=False))

        model_rows.append({
            "model": model_name, "feature": feature,
            "q1_low_mean_signed_shap":  q1_signed,
            "q5_high_mean_signed_shap": q5_signed,
            "q5_minus_q1": q5_signed - q1_signed,
            "high_value_direction": high_value_direction
        })

    quintile_results[model_name] = pd.DataFrame(model_rows)

quintile_summary = pd.concat(quintile_results.values(), ignore_index=True)
print(f"\nSummary: high-value direction by model and feature")
print(quintile_summary.round(6).to_string(index=False))

quintile_summary.to_csv("output/results/h3a_signed_shap_quintile_summary.csv", index=False)
print(f"Saved: output/results/h3a_signed_shap_quintile_summary.csv")
```

    H3a: signed SHAP quintile diagnostic — foreign flow variables
    
    RF Block 3
    
      Feature: f_sell_5d  |  high-value direction: high values push predictions downward
    quintile  mean_feature_value  mean_signed_shap  mean_abs_shap  n_obs
      Q1 low           -0.398903          0.000920       0.000991  19596
          Q2           -0.204209         -0.000193       0.000198  19601
          Q3            0.000068         -0.000186       0.000187  19596
          Q4            0.201109         -0.000183       0.000185  19591
     Q5 high            0.402120         -0.000177       0.000180  19592
    
      Feature: f_buy_5d  |  high-value direction: high values push predictions downward
    quintile  mean_feature_value  mean_signed_shap  mean_abs_shap  n_obs
      Q1 low           -0.401583          0.000076       0.000164  19601
          Q2           -0.201508         -0.000009       0.000028  19591
          Q3            0.000046         -0.000007       0.000026  19601
          Q4            0.201109         -0.000010       0.000027  19591
     Q5 high            0.402120         -0.000064       0.000072  19592
    
    XGB Block 3
    
      Feature: f_sell_5d  |  high-value direction: high values push predictions downward
    quintile  mean_feature_value  mean_signed_shap  mean_abs_shap  n_obs
      Q1 low           -0.398903          0.001276       0.001499  19596
          Q2           -0.204209         -0.000408       0.000419  19601
          Q3            0.000068         -0.000290       0.000292  19596
          Q4            0.201109         -0.000276       0.000278  19591
     Q5 high            0.402120         -0.000231       0.000264  19592
    
      Feature: f_buy_5d  |  high-value direction: high values push predictions downward
    quintile  mean_feature_value  mean_signed_shap  mean_abs_shap  n_obs
      Q1 low           -0.401583          0.000170       0.000490  19601
          Q2           -0.201508         -0.000007       0.000070  19591
          Q3            0.000046          0.000009       0.000071  19601
          Q4            0.201109         -0.000005       0.000084  19591
     Q5 high            0.402120         -0.000326       0.000349  19592
    
    Summary: high-value direction by model and feature
    model   feature  q1_low_mean_signed_shap  q5_high_mean_signed_shap  q5_minus_q1                  high_value_direction
       RF f_sell_5d                 0.000920                 -0.000177    -0.001097 high values push predictions downward
       RF  f_buy_5d                 0.000076                 -0.000064    -0.000140 high values push predictions downward
      XGB f_sell_5d                 0.001276                 -0.000231    -0.001507 high values push predictions downward
      XGB  f_buy_5d                 0.000170                 -0.000326    -0.000496 high values push predictions downward
    Saved: output/results/h3a_signed_shap_quintile_summary.csv
...
![png](04_evaluation_files/04_evaluation_21_1.png)
...
![png](04_evaluation_files/04_evaluation_24_1.png)
